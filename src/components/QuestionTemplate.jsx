      ...
      <div className="mb-8 flex items-start justify-between">
        <div className="flex-grow mr-4">
          <div className="flex items-center mb-2">
            {citation ? (
              <h2 className="text-2xl font-bold text-left mr-2">
                {question}{" "}
                <span className="text-teal-400 italic">"{citation}"</span>
              </h2>
            ) : (
        ...
        {options.map((option, index) =>
          figures ? (
            ...
            <div
              key={index}
              onClick={() => handleOptionClick(option)}
              className={`w-full bg-gray-800 hover:bg-gray-700 py-3 px-4 rounded-lg transition duration-300 flex items-center justify-between cursor-pointer
    ${
